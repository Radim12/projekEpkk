  ...
  <table class="table">
    ...
    <tbody>
      ...
      <tr>
        ...
        <td>{{ $umum->tanggal }}</td>
        <td>
             <a href="{{ route('bidangumum.edit', $umum->id_laporan_umum) }}" class="btn btn-sm btn-tambah">Review</a>
<br><br>
            <form action="{{ route('bidangumum.destroy', $umum->id_laporan_umum)}}" method="POST" class="d-inline">
              @csrf
              @method('DELETE')
